...
char* num_to_airport(int point) {
	...
	if (point == 0) {
		...
		return "PDX"; //Assume they are flying out of Portland. 
	}
}
//Dijkstras:
void dijkstra(graph_* graph, int point) { //removed const char* airport
	int n = graph->n_nodes;
	...
	// Main loop: extract the vertex with the smallest distance and relax its edges.
	while (!pq_isempty(pq)) {
		...
		// Skip this node if the distance is outdated.
		if (d > dist[u])
			continue;
	...
	printf("Dijkstra's from %d:\n", point);
	printf("End point       ||  Lowest Cost Path \n");
	for (int i = 0; i < n; i++) {
		// char* air = num_to_airport(i); 
		printf("%d		||		%d\n", i, dist[i]);
	}
...
int main(int argc, char const *argv[]) {
	...
	*/
	...
	dijkstra(graph, start); //Removed airport 
